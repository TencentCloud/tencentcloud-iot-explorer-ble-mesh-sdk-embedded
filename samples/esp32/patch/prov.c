...
extern int llsync_mesh_auth_clac(uint8_t *random, uint8_t *auth_data);
static int prov_auth(u8_t method, u8_t action, u8_t size)
{
	...
	case AUTH_METHOD_STATIC:
		...
		llsync_mesh_auth_clac(link.rand, link.auth + 16 - prov->static_val_len);
		...
		BT_ERR("sizeof(link.auth) : %d", sizeof(link.auth));
		BT_ERR("link.auth: %s",  bt_hex(link.auth, 16));
		return 0;
...
static void prov_random(const u8_t *data)
{
	...
	if (memcmp(conf_verify, link.conf, 16)) {
		...
		BT_ERR("link.auth: %s",  bt_hex(link.auth, 16));
		BT_ERR("Received:   %s", bt_hex(link.conf, 16));
		BT_ERR("Calculated: %s",  bt_hex(conf_verify, 16));
...
int bt_mesh_pb_gatt_recv(uint16_t conn_handle, struct os_mbuf *buf)
{
	...
	BT_WARN("recv <-- %u bytes: %s", buf->om_len, bt_hex(buf->om_data, buf->om_len));
