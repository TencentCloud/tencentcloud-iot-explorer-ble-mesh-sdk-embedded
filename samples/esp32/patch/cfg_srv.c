			...
			struct os_mbuf *buf)
{
	...
	BT_WARN("elem_addr 0x%04x pub_addr 0x%04x cred_flag %u",
			...
			struct os_mbuf *buf)
{
	...
	BT_WARN("11111111111111111111111111111111s");
	...
	BT_WARN("elem_addr 0x%04x, sub_addr 0x%04x", elem_addr, sub_addr);
	...
	if (bt_mesh_model_find_group(&mod, sub_addr)) {
		...
		BT_WARN("found existing subscription");
		status = STATUS_SUCCESS;
		goto send_status;
	}
			    ...
			    struct os_mbuf *buf)
{
	...
	BT_WARN("addr 0x%04x company 0x%04x id 0x%04x", addr, company, id);
			   ...
			   struct os_mbuf *buf)
{
	...
	BT_WARN("222222222222222222222222222222");
