...
#define BLE_MESH_DEVICE_UUID_LEN  (16)
#define BLE_MESH_RAMDOM_DATA_LEN  (16)
#define BLE_MESH_DEV_NAME_LEN     (12)
#define BLE_MESH_ADV_MAX_LEN      (31)

#define BLE_MESH_CID_VENDOR_LOW   (LLSYNC_MESH_CID_VENDOR & 0xff)

#define BLE_MESH_CID_VENDOR_HIGH  ((LLSYNC_MESH_CID_VENDOR >> 8) & 0xff)
...
// 定时器周期(ms)
#define BLE_MESH_TIMER_PERIOD       (10)
...
#define BLE_MESH_RESP_DATA_MAX_LEN  (1024)

#define BLE_MESH_RECV_MAX_TID       (0x7F)

#define BLE_MESH_REPORT_MAX_TID     (0xBF)

#define BLE_MESH_REPORT_MIN_TID     (0x80)

#define BLE_GET_EXPIRATION_TIME(_cur_time) ((_cur_time) + 60)
...
typedef struct {
    uint8_t  flags[3];
    uint8_t  service_uuid_list[4];
    uint8_t  length;
    uint8_t  type;
    uint16_t mesh_seivice;
}__attribute__((packed)) llsync_mesh_unnet_head_t;

typedef struct {
    uint8_t  flags[3];
    uint8_t  length;
    uint8_t  type;
}__attribute__((packed)) llsync_mesh_scan_head_t;

typedef void *ble_timer_t;
typedef struct {
    uint8_t  adv[BLE_MESH_ADV_MAX_LEN];
    uint8_t  data_len;
    uint8_t  adv_status;
    uint8_t  timer_type;
    uint16_t  timer_cnt;
