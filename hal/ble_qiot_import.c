...
// extern esp_err_t esp_ble_gap_config_adv_data_raw(uint8_t *raw_data, uint32_t raw_data_len);
// extern esp_err_t esp_ble_gap_stop_advertising(void);

#define PRODUCT_ID  "N0HCN11GM8"
#define DEVICE_NAME "aabbccddeeff"   //注意此处的dev name使用设备的mac地址来命名
#define DEVICE_SECRET "8QX64wm6Y80n1DKPQgxIqw=="
...
ble_qiot_ret_status_t llsync_mesh_dev_info_get(ble_device_info_t *dev_info)
{
    ...
    memcpy(dev_info->psk, DEVICE_SECRET, strlen(DEVICE_SECRET));
...
// 根据opcode将此数据发送出去，主要有status和indication两种
